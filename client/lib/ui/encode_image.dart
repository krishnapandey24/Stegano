  ...
  mainContainer(bool isWeb) {
    if (imageEncoded) {
      return EncodeDownload(
          isWeb: isWeb, setState: setImage, fileBytes: encodedImageBytes!);
    } else {
    ...
    Row buttonRow = Row(
      ...
      children: [
        ElevatedButton(
          onPressed: () async {
            try {
              Appt.showLoading(context, "Encoding your image...");
              encodedImageBytes = await api.encodeImageForWeb(
                  webImage, textController.text, false);
              setState(() {
                imageEncoded = true;
              });
            } catch (e) {
